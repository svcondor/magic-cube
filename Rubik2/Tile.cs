...
namespace Rubik2
{
  public class Tile : MyModelVisual3D
  {
    public Color color;
    ...
    public Tile(int x, int y, int TileIx, Color color) {
      ...
      drawTile(p0, p1, p2, p3, TileIx, color);
    }
    ...
    void drawTile(Point3D p0, Point3D p1, Point3D p2, Point3D p3, int tileIx, Color color) {
      Material tempMaterial = new DiffuseMaterial(new SolidColorBrush(color));

      //  Debug.WriteLine($"Blackface {p0} {p1} {p2} {p3} ");
      ModelVisual3D r1 = new ModelVisual3D();
      ModelVisual3D r2 = new ModelVisual3D();

      var t1 = Helpers.createTriangleModel(p0, p1, p2, tempMaterial);
      var t2 = Helpers.createTriangleModel(p0, p2, p3, tempMaterial);
      r1.Content = t1;
      r2.Content = t2;
      this.Children.Add(r1);
      this.Children.Add(r2);
