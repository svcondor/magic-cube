...
namespace Rubik2
{
  public class Cube2D
  {
    private int sidePieces;
    public CubeFace[,] projection { get; private set; }

    public Cube2D() {
      this.sidePieces = Cube.sidePieces;
    ...
    private void createCube() {
      for (int i = 0; i < sidePieces * 4; i++) {
        for (int j = 0; j < sidePieces * 3; j++) {
          ...
          else if (i >= sidePieces && i < sidePieces * 2) {
            if (j < sidePieces) {
              projection[i, j] = CubeFace.L;
            }
            else if (j >= sidePieces && j < sidePieces * 2) {
          ...
          else if (i >= sidePieces * 2 && i < sidePieces * 3 && j >= sidePieces && j < sidePieces * 2) {
            projection[i, j] = CubeFace.F;
          }
          else if (i >= sidePieces * 3 && j >= sidePieces && j < sidePieces * 2) {
    ...
    private bool isUnscrambledB() {
      CubeFace f = projection[0, sidePieces];

      for (int i = 0; i < sidePieces; i++) {
        for (int j = sidePieces; j < sidePieces * 2; j++) {
          if (projection[i, j] != f) {
            return false;
          }
        }
    ...
    private bool isUnscrambledD() {
      CubeFace f = projection[sidePieces, sidePieces];

      for (int i = sidePieces; i < sidePieces * 2; i++) {
        for (int j = sidePieces; j < sidePieces * 2; j++) {
          if (projection[i, j] != f) {
            return false;
          }
    ...
    private bool isUnscrambledF() {
      CubeFace f = projection[sidePieces * 2, sidePieces];

      for (int i = sidePieces * 2; i < sidePieces * 3; i++) {
        for (int j = sidePieces; j < sidePieces * 2; j++) {
          if (projection[i, j] != f) {
            return false;
          }
    ...
    private bool isUnscrambledU() {
      CubeFace f = projection[sidePieces * 3, sidePieces];

      for (int i = sidePieces * 3; i < sidePieces * 4; i++) {
        for (int j = sidePieces; j < sidePieces * 2; j++) {
          if (projection[i, j] != f) {
            return false;
          }
    ...
    private bool isUnscrambledL() {
      CubeFace f = projection[sidePieces, 0];

      for (int i = sidePieces; i < sidePieces * 2; i++) {
        for (int j = 0; j < sidePieces; j++) {
          if (projection[i, j] != f) {
            return false;
          }
        }
    ...
    private bool isUnscrambledR() {
      CubeFace f = projection[sidePieces, sidePieces * 2];
      for (int i = sidePieces; i < sidePieces * 2; i++) {
        for (int j = sidePieces * 2; j < sidePieces * 3; j++) {
          if (projection[i, j] != f) {
            return false;
          }
        }
